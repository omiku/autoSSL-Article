...
func (tx *Tx) init() {
	...
	tx.CertificateDeployment = NewCertificateDeploymentClient(tx.config)
	tx.DNSProvider = NewDNSProviderClient(tx.config)
	tx.GlobalNotificationConfig = NewGlobalNotificationConfigClient(tx.config)
	...
	tx.ServerGroup = NewServerGroupClient(tx.config)
	tx.ServerGroupMember = NewServerGroupMemberClient(tx.config)
