...
type AccessQuery struct {
	...
	withUser               *UserQuery
	withServerGroupMembers *ServerGroupMemberQuery
...
// QueryServerGroupMembers chains the current query on the "server_group_members" edge.
func (aq *AccessQuery) QueryServerGroupMembers() *ServerGroupMemberQuery {
	query := (&ServerGroupMemberClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccessQuery) QueryDeployments() *CertificateDeploymentQuery {
	query := (&CertificateDeploymentClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccessQuery) Clone() *AccessQuery {
	...
	return &AccessQuery{
		...
		withServerGroupMembers: aq.withServerGroupMembers.Clone(),
...
// WithServerGroupMembers tells the query-builder to eager-load the nodes that are connected to
// the "server_group_members" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccessQuery) WithServerGroupMembers(opts ...func(*ServerGroupMemberQuery)) *AccessQuery {
	query := (&ServerGroupMemberClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withServerGroupMembers = query
	return aq
}
...
func (aq *AccessQuery) WithDeployments(opts ...func(*CertificateDeploymentQuery)) *AccessQuery {
	query := (&CertificateDeploymentClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccessQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Access, error) {
	...
	if query := aq.withServerGroupMembers; query != nil {
		if err := aq.loadServerGroupMembers(ctx, query, nodes,
			func(n *Access) { n.Edges.ServerGroupMembers = []*ServerGroupMember{} },
			func(n *Access, e *ServerGroupMember) {
				n.Edges.ServerGroupMembers = append(n.Edges.ServerGroupMembers, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := aq.withDeployments; query != nil {
		if err := aq.loadDeployments(ctx, query, nodes,
			func(n *Access) { n.Edges.Deployments = []*CertificateDeployment{} },
			func(n *Access, e *CertificateDeployment) { n.Edges.Deployments = append(n.Edges.Deployments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AccessQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Access, init func(*Access), assign func(*Access, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_access_configs == nil {
			continue
		}
		fk := *nodes[i].user_access_configs
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_access_configs" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AccessQuery) loadServerGroupMembers(ctx context.Context, query *ServerGroupMemberQuery, nodes []*Access, init func(*Access), assign func(*Access, *ServerGroupMember)) error {
	...
	query.Where(predicate.ServerGroupMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(access.ServerGroupMembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.access_server_group_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "access_server_group_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "access_server_group_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccessQuery) loadDeployments(ctx context.Context, query *CertificateDeploymentQuery, nodes []*Access, init func(*Access), assign func(*Access, *CertificateDeployment)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Access)
	nids := make(map[int]map[*Access]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(access.DeploymentsTable)
		s.Join(joinT).On(s.C(certificatedeployment.FieldID), joinT.C(access.DeploymentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(access.DeploymentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(access.DeploymentsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*CertificateDeployment](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "deployments" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
