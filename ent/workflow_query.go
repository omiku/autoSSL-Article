...
func (wq *WorkflowQuery) QueryCertificate() *CertificateQuery {
	query := (&CertificateClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) QueryUser() *UserQuery {
...
func (wq *WorkflowQuery) QuerySteps() *WorkflowStepQuery {
	query := (&WorkflowStepClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wq *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withCertificate: wq.withCertificate.Clone(),
		withUser:        wq.withUser.Clone(),
		withSteps:       wq.withSteps.Clone(),
...
func (wq *WorkflowQuery) WithCertificate(opts ...func(*CertificateQuery)) *WorkflowQuery {
	query := (&CertificateClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withCertificate = query
	return wq
}

// WithUser tells the query-builder to eager-load the nodes that are connected to
// the "user" edge. The optional arguments are used to configure the query builder of the edge.
func (wq *WorkflowQuery) WithUser(opts ...func(*UserQuery)) *WorkflowQuery {
	query := (&UserClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (wq *WorkflowQuery) WithSteps(opts ...func(*WorkflowStepQuery)) *WorkflowQuery {
	query := (&WorkflowStepClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	var (
		...
		_spec       = wq.querySpec()
		loadedTypes = [3]bool{
			wq.withCertificate != nil,
			wq.withUser != nil,
			wq.withSteps != nil,
		}
	)
	if wq.withCertificate != nil || wq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := wq.withCertificate; query != nil {
		if err := wq.loadCertificate(ctx, query, nodes, nil,
			func(n *Workflow, e *Certificate) { n.Edges.Certificate = e }); err != nil {
			return nil, err
		}
	}
	if query := wq.withUser; query != nil {
		if err := wq.loadUser(ctx, query, nodes, nil,
			func(n *Workflow, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := wq.withSteps; query != nil {
		if err := wq.loadSteps(ctx, query, nodes,
			func(n *Workflow) { n.Edges.Steps = []*WorkflowStep{} },
			func(n *Workflow, e *WorkflowStep) { n.Edges.Steps = append(n.Edges.Steps, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WorkflowQuery) loadCertificate(ctx context.Context, query *CertificateQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Certificate)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Workflow)
	for i := range nodes {
		if nodes[i].certificate_workflows == nil {
			continue
		}
		fk := *nodes[i].certificate_workflows
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "certificate_workflows" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (wq *WorkflowQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Workflow)
	for i := range nodes {
		if nodes[i].user_workflows == nil {
			continue
		}
		fk := *nodes[i].user_workflows
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_workflows" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (wq *WorkflowQuery) loadSteps(ctx context.Context, query *WorkflowStepQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *WorkflowStep)) error {
	...
	for _, n := range neighbors {
		fk := n.workflow_steps
		if fk == nil {
			return fmt.Errorf(`foreign-key "workflow_steps" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workflow_steps" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
